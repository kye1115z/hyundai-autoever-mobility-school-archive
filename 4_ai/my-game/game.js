...
function updatePlayer1() {
    ...
    if (playerIndex === 0 || gameMode === '1P') {
        ...
        if (player1.x + player1.width / 2 > net.x) player1.x = net.x - player1.width / 2;
         ...
         if (player1.x - player1.width / 2 < net.x + net.width) player1.x = net.x + net.width + player1.width/2;
         if (player1.x + player1.width / 2 > canvasWidth) player1.x = canvasWidth-player1.width/2;
    }
...
function updatePlayer2AI() {
    ...
    if (ball.y < player2.y - player2.height && ball.x > player2.x - 50 && ball.x < player2.x + 50 && player2.y === groundY) {
...
function updateBall() {
    ...
    if (gameMode === '1P' || (gameMode === '2P' && playerIndex === 0)) {
        ...
        if (ball.y + ball.radius > groundY) {
             if (ball.x < canvasWidth / 2) { // player 2 진영
                player2Score++;
            } else { // player 1 진영
                player1Score++;
            }
    ...
    ws.onmessage = (event) => {
        ...
        switch (type) {
            ...
            case 'update_opponent':
                ...
                break;
            case 'update_ball':
                if (playerIndex !== 0) { // 방장이 아니면 공 위치 업데이트
                    ball.x = payload.x;
                    ball.y = payload.y;
                    ball.vx = payload.vx;
                    ball.vy = payload.vy;
